...
$(document).ready(function(){
	...
	if (isset($_GET['key'])) {
		$get_sql = "select * from tbl_posts where post_status = 1 post_name like '%{$search}%' order by post_id desc limit {$start} , {$perpage} ";
	} else {
		...
		while ($row=mysqli_fetch_array($result)) {
			...
			?>
			   ...
			   <div class="col-sm-6 col-md-4">
				<div  style="text-align:center;width:100%;border-style: solid;border-width: 1px;border-color: #e3e3e3;">
					...
					<div class="caption" style="margin-top:7px;">
						...
						<div class="dv_post_desc_short">
							<?php echo iconv_substr(strip_tags(htmlspecialchars_decode($row["post_desc"])),0,200, "UTF-8"); ?>...
							 <p><button class="btn btn-default" type="button">Read More..</button></p>
						</div>
					</div>
				</div>
			</div>
