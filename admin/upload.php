...
if ($method == "POST") {
    ...
    // Handles upload requests
        ...
        // insert data to table tbl_file_doc

        if (($result["ext"] == "jpg") or ($result["ext"] == "jpeg") or ($result["ext"] == "png") or ($result["ext"] == "gif")) {
          $file_type = "1";
        } else {
          $file_type = "2";
        }

        $sql = "INSERT INTO tbl_file_doc (file_name, full_name, folder_name, file_type) VALUES ('".$result["uploadName"]."','".$result["uploadName"]."','".  $result["uuid"]."',".$file_type.")";

        $query = mysqli_query($con,$sql);
        $result["res_ins"] = $query;
    }
